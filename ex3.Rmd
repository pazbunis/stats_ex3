...
important_feats = matrix(0,nrow = 128, ncol = 128)
for (i in 1:N) {
  important_feats = important_feats + t(matrix(Re(wav.pyr[,topN[i]]), nrow = 128)[128:1,]);
}
image(important_feats,col = grey.colors(100))
...
```{r}
bestFeature = topN[1]
plot(X[,bestFeature], y[,best_voxel_num])
